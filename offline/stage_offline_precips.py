...
# Antigua training package — only this cycle has offline STREAM-Sat + StormLab.
# Override with env TITO_OFFLINE_ALLOWED_CYCLES=202510100100
DEFAULT_OFFLINE_CYCLES = (
    "202510100100",  # 2025-10-10 01:00 UTC
)
...
def validate_offline_cycle(cycle_key: str, source: Optional[Path] = None) -> None:
    ...
    lines.append(
        '  Use e.g.:  ./tito-run.sh hindcast "2025-10-10 01:00" '
        '"2025-10-10 01:00" --regions Antigua --offline'
    )
    lines.append(
...
def _stormlab_domain(region: str) -> str:
    try:
        from tito_utils.qpf_utils.stormlab_utils import get_stormlab_domain
        return get_stormlab_domain(region)
    except Exception:
        return str(region or "").strip().lower() or "guatemala"


def _resolve_sl_root(source: Path, region_slug: str = "lesserantilles") -> Path:
    sl_root = source / "stormlab"
    for slug in (region_slug, "lesserantilles", "antigua", "guatemala"):
        cand = sl_root / slug
        if cand.is_dir() and any(cand.glob("ensQ*")):
            return cand
    if sl_root.is_dir() and any(sl_root.glob("ensQ*")):
        return sl_root
    raise FileNotFoundError(f"No StormLab ensQ* under {sl_root}")
...
def stage(
    *,
    ...
    ss_ens: int,
    sl_ens: int,
    cycle_key: str = "202510100100",
    region_slug: str = "lesserantilles",
) -> dict:
    """
...
def _stage_from_ef5_layout(root: Path, *, ss_ens: int, sl_ens: int, cycle_key: str):
    ...
    sl_src = None
    sl_parent = precip / "stormlab"
    if sl_parent.is_dir():
        for cand in (sl_parent / "lesserantilles", sl_parent / "antigua", sl_parent / "guatemala"):
            if cand.is_dir() and any(cand.glob("ensQ*")):
                sl_src = cand
                break
        if sl_src is None:
            for cand in sorted(sl_parent.iterdir()):
                if cand.is_dir() and any(cand.glob("ensQ*")):
                    sl_src = cand
                    break
    if sl_src is not None and sl_src.is_dir():
    ...
    if imerg.is_dir() and any(imerg.glob("*.tif")):
        for ck in {cycle_key, "202510100100"}:
