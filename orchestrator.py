...
def main(args):
    ...
    _QPE_ALIASES = {
        "STREAMSAT": "STREAM_SAT",
        "STREAM-SAT": "STREAM_SAT",
        "STREAM_SAT": "STREAM_SAT",
    }

    def _canon_qpe(token):
        c = str(token).strip().upper()
        return _QPE_ALIASES.get(c, c)

    def _normalize_qpe(val, fallback):
        if isinstance(val, (list, tuple)):
            out = [_canon_qpe(x) for x in val]
            out = [v for v in out if v in _VALID_QPE]
            if not out:
                print(f"    WARNING: unknown qpe_source {val!r} — using {fallback}")
                return [fallback]
            return out
        c = _canon_qpe(val)
        if c in _VALID_QPE:
    ...
    if HindCastMode:
        arome_regions = [
            r for r, srcs in region_qpf_requested.items() if "AROME" in srcs]
        if arome_regions:
            raise RuntimeError(
                "AROME cannot be used in hindcast (no archive). "
                "Remove AROME from qpf_source in region_forcing_map "
                f"for: {', '.join(arome_regions)}. "
                "Use STORMLAB or GFS for hindcast, or run operational mode."
            )
        region_cycle_times = {}
        hc_dates = []
