        ...
        if actual == "STORMLAB":
            ...
            for sl_idx in range(1, sl_ens + 1):
                ...
                if not os.path.isdir(sl_member_dir):
                    continue
                tmp_out = _cycle_product_dir(
                    config, cfg, "stormlab", f"ensOut{sl_idx}")
                ...
                try:
                    job_log = setup_run_log(
                        log_dir, f"ef5_stormlab_qpe_{rkey}_sl{sl_idx:02d}")
                    _, ctrl_file, run_path = prepare_ef5(
                        staging,
                        sl_member_dir,
                        ...
                        ct,
                        ctx["systemName"],
                        ctx["SEND_ALERTS"], ctx["alert_recipients"],
                        ctx["smtp_config"],
                        with_sep(tmp_out),
                        with_sep(log_dir),
                        region, ctx["systemModel"],
                    ...
                    with batch.lock:
                        ...
                        batch.lr_jobs.append(_job_dict(
                            region, ctx["ef5Path"], run_path, ctrl_file,
                            cfg["output_timestamp_str"], member=sl_idx))
                except Exception as exc:
        ...
        if legacy_qpf:
            ...
            for qpf_src in legacy_qpf:
                ...
                try:
                    ...
                    print(f"    {region} [SS ens{member_idx:02d} {actual_qpf} QPE]: "
